...
class QAgeNet:
    ...
    def __init__(self, num_qubits=3, num_executions=150, 
        ...
        Args:
            ...
            contamination: Expected fraction of aged/anomalous executions
        """
    ...
    def extract_features(self, execution_num, fidelity, counts, entropy):
        """Extract features for ML aging detection"""
        count_values = np.array(list(counts.values()))
        
        features = [
            fidelity,                                    
            np.std(count_values) / self.shots,          
            len(counts),                                 
            max(count_values) / self.shots,             
            entropy,                                     
            execution_num / self.num_executions,        
            self.aging_model.aging_factor,              
            np.mean(count_values) / self.shots,         
        ]
    ...
    def run_experiment(self):
        ...
        print("\n" + "="*70)
        print("   Q-AGENET v3.2: QUANTUM CIRCUIT AGING DETECTOR")
        print("   [FIXED VERSION - Simple GHZ Circuit for High Fidelity]")
        print("="*70)
        ...
        print(f"  Expected Aging Rate: {self.contamination*100:.1f}%")
        print("="*70)
        ...
        for exec_num in range(self.num_executions):
            ...
            features = self.extract_features(exec_num, fidelity, counts, entropy)
        ...
        # Train ML detector
        print("\n" + "="*70)
        print("Training ML Aging Detector...")
        print("="*70)
        
        X = np.array(feature_matrix[20:])
        X_scaled = self.scaler.fit_transform(X)
        
        # Fit anomaly detector
        self.detector.fit(X_scaled)
        self.predictions = self.detector.predict(X_scaled)
        
        # -1 = anomaly (aged), 1 = normal
        # Pad aging detection to match full execution length
        burn_in = 20
        aging_detected_trimmed = (self.predictions == -1)

        aging_detected = np.zeros(self.num_executions, dtype=bool)
        aging_detected[burn_in:] = aging_detected_trimmed

        
        print(f"✓ Detector trained on {len(X)} execution samples")
        print(f"✓ Anomalies detected: {np.sum(aging_detected)}/{len(aging_detected)}")
    ...
    def analyze_results(self, aging_detected):
        ...
        early_false_positives = int(np.sum(aging_detected[:50]))
        false_positive_rate = float((early_false_positives / 50) * 100)
        ...
        success_criteria = {
            ...
            'low_false_positives': bool(false_positive_rate < 30)
        }
        ...
        print(f"  False Positive Rate (early): {false_positive_rate:.1f}% {'✓' if success_criteria['low_false_positives'] else '✗'}")
        ...
        if not success_criteria['detection_accuracy_good']:
            ...
            print(f"     → Adjust contamination or model parameters")
        else:
        ...
        if not success_criteria['low_false_positives']:
            ...
            print(f"     → Reduce contamination parameter")
        else:
    ...
    def visualize_results(self, aging_detected):
        ...
        ax2.plot(executions, self.cai_values, 'g-', linewidth=2.5, alpha=0.8)
        ax2.fill_between(executions, 0, self.cai_values, alpha=0.3, color='green')
...
def main():
    ...
    qagenet = QAgeNet(
        ...
        num_executions=150,       
        shots=2048,               
        contamination=0.15        # 22% aging expected
    )
    ...
    if results['metrics']['overall_success_score'] >= 75:
        ...
        print("   → ML detector shows good accuracy")
